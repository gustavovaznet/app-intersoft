<?php
	...
	if($action == 'insert'){
		...
		if(empty($_POST['product']) || empty($_POST['model']) || empty($_POST['amount']) || empty($_POST['snumber']) || empty($_POST['function']) || empty($_POST['company'])){
	...
	//RECOVER FOR UPDATE
		...
		$product->__set('id',$_GET['id']);
		$productService = new ProductService($connection, $product);
		$products = $productService->recover_for_update();
